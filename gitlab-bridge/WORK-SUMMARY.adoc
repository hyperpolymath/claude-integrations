...
- Added new badges:
  * MPL-2.0-or-later license badge
  * AffineScript language badge
  * Deno runtime badge
  * Idris-ready badge

=== ✓ Task 2: Converted TypeScript to AffineScript

**Scope:** Complete codebase conversion from TypeScript to AffineScript
...
|`src/auth/Errors.res`
|Error types using AffineScript exception system
...
|Anthropic SDK bindings for AffineScript

|`src/bindings/Express.res`
|Express.js bindings for AffineScript

|`affinescript.json`
|AffineScript compiler configuration
...
4. *Fast Compilation:* AffineScript compiler is much faster than TypeScript
5. *Deno Runtime:* Modern runtime with built-in TypeScript support

**Language Policy Compliance:**

- ✅ No TypeScript (converted to AffineScript)
- ✅ No Node.js (using Deno)
- ✅ No npm (using Deno imports)
- ✅ AffineScript as primary language
...
- Changed code examples to AffineScript syntax
- Updated project structure to show AffineScript files
- Updated directory tree to show `bindings/` instead of `middleware/`

=== License Updates

**All files updated to:**
- License: MPL-2.0-or-later (MPL-2.0)
...
**Build:**
- `affinescript@^12.0.2` - AffineScript compiler
- `@affinescript/core@^1.0.0` - AffineScript standard library
...
├── deno.json                 # Deno config (package manifest;
│                             #   package.json removed — npm is banned)
...
* AffineScript provides superior type safety over TypeScript
* Deno eliminates need for node_modules and npm
* MPL-2.0 (MPL-2.0-or-later) applied throughout
