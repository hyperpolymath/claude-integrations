...
= Migration Guide: TypeScript to AffineScript
:toc:
:toclevels: 3

== Overview

Claude GitLab Bridge has been migrated from TypeScript to AffineScript, following the hyperpolymath language policy. This document explains the changes and how to use the new codebase.
...
2. Install AffineScript:
[source,bash]
----
deno install npm:affinescript@^12.0.2
----
...
If you need to use both TypeScript and AffineScript during transition:

1. AffineScript compiles to clean JavaScript
2. Compiled `.res.js` files can be imported from TypeScript
3. Use `gentypeconfig` in `affinescript.json` for TypeScript definitions

== Resources

* https://affinescript-lang.org/docs/manual/latest/introduction[AffineScript Documentation]
